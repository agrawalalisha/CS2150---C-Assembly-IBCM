...
; Return type is void 
mergeSort: 
	cmp rsi, rdx		; rsi = left  rdx = right
	jge end			; if( left >= right)goto end

	mov rcx, rdx		; rcx = right, rdx = middle
	add rdx, rsi		; rdx = middle (right + left)
	shr rdx, 1		; rdx = middle (rihgt + left) / 2

	push rsi		; save left
	push rcx		; save right
	
	call mergeSort		; rdi, rsi, rdx

	mov rsi, rdx		; rsi = middle
	add rsi, 1		; rsi = middle + 1
	...
	pop rcx			; rcx = right
	mov rdx, rsi		; rdx = middle + 1
	sub rdx, 1		; rdx = middle

	pop rsi			; rsi = left

	push r10		
	push r11
	call merge		; rdi(array), rsi(left), rdx (middle). rcx(right)
	pop r11
	pop r10
